...
const VerticalCardProduct = ({category, heading}) => {
    ...
    const fetchData = async() =>{
        ...
        try {
            const categoryProduct = await fetchCategoryWiseProduct(category)
            // Ensure that we handle the case where `categoryProduct.data` is undefined
    ...
    <div className='container mx-auto px-4 my-6 relative'>
            ...
            <div className='flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
                ...
                {loading ? (
                    ...
                    // Conditional rendering with a check to ensure `data` is a valid array
                    Array.isArray(data) && data.length > 0 ? (
                        ...
                        <div>No products available</div> // Optional: display a fallback message if no data
                    )
                )}
            </div>
        </div>
