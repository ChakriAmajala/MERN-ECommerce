...
const HorizontalCardProduct = ({category, heading}) => {
    ...
    const fetchData = async() =>{
        ...
        try {
            const categoryProduct = await fetchCategoryWiseProduct(category)
            if (categoryProduct?.data && Array.isArray(categoryProduct.data)) {
                setData(categoryProduct.data) // Ensure the data is an array
            } else {
                setData([]) // If no valid data, set empty array
            }
        } catch (error) {
            ...
            <div className='flex items-center gap-4 md:gap-6 overflow-scroll scrollbar-none transition-all' ref={scrollElement}>
                ...
                {loading ? (
                    ...
                    data?.length > 0 ? (
                        ...
                        <div>No products available</div> // Fallback message if no products found
                    )
                )}
            </div>
        </div>
